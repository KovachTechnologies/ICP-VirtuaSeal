...
const internetIdentityDisplay = ref();
const isHovered = useElementHover(internetIdentityDisplay);
...
const headers: Header[] = [
    ...
    { text: 'Recievers', value: 'recievers' },
    { text: 'CC', value: 'copied' },
];

const onClickRow = async (row: any) => {
    documentStore.onGetSingleDocument(row.id);
};
</script>
...
<template>
    <div class="flex flex-col gap-y-8">
        <div class="border-b border-secondary pb-5">
            <h1 ref="internetIdentityDisplay" class="text-brand-secondary" :class="!isHovered && 'max-w-xl truncate'">
        ...
        <div>
            <div class="flex justify-between">
                <div>
                    <h2>All Documents</h2>
                    <p>Keep track of your documents and their status.</p>
                </div>
                <div class="flex gap-x-2">
                    <u-btn
                        ...
                        color="gray"
                        icon="refresh-cw-05"
                        :loading="documentStore.loadingDocumentState.table"
                        @click="documentStore.onGetDocuments()"
                    />
        ...
        <EasyDataTable
            ...
            :headers="headers"
            :items="documentStore.myDocuments"
            :loading="documentStore.loadingDocumentState.table"
            :sort-by="sortBy"
            :sort-type="sortType"
        ...
        >
            <template #loading>
                <div class="flex flex-col gap-4 items-center justify-center text-center">
                    <u-spinner size="2xl" />
                    <p class="text-sm font-medium text-secondary">Loading documents...</p>
                </div>
            </template>
